...
def main(args, init_distributed=False):
    ...
    for valid_sub_split in args.valid_subset.split(','):
            task.load_dataset(valid_sub_split, combine=False, epoch=1, max_tokens=25000)
            dataset.truncate(max_tokens=25000)
...
def train(args, trainer, task, epoch_itr, max_update=math.inf):
    ...
    # with open('/home/mzzhong/waitk/scripts/wmt15deen/wandb.yaml', 'r') as f:
    #     config = yaml.safe_load(f)
    # wandb.init(project='Wait-k', name=config['name'])
    if trainer.get_num_updates() == 0:
    ...
    for samples in progress:
        with metrics.aggregate('train_inner'):
            ...
            if log_output is None:  # OOM, overflow, ...
                continue
            # else:
            #     wandb.log(log_output, step=trainer.get_num_updates())
